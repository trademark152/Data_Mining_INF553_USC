...
"""
To run code: spark-submit hw1/minh_tran_task2.py yelp_dataset/review.json yelp_dataset/business.json output1.txt output2.txt
"""
...
def task2(res_task2, output_file_path2):
    ...
    s_a = time.time()
    res_task2_a = res_task2.map(lambda x: (x[0], x[1][0] / x[1][1])).sortBy(lambda x: (-x[1], x[0]),
                                                                            ascending=True).collect()[0:5]
    print(res_task2_a)
    e_a = time.time()
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "This function needs 4 input arguments <input_file_name1> <input_file_name2> <output_file_name1> <output_file_name2>")
        sys.exit(1)
