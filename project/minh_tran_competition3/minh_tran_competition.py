...
if __name__ == "__main__":
    ...
    '''
    bsl_options = {'method': 'als','n_epochs': 7, 'reg_u': 5, 'reg_i': 3}
    print(bsl_options)
    algo = BaselineOnly(bsl_options=bsl_options)
    algo.fit(trainset)

    # for sgd
    bsl_options1 = {'method': 'sgd', 'learning_rate': .005}
    algo1 = BaselineOnly(bsl_options=bsl_options1)
    print(bsl_options1)
    algo1.fit(trainset)
    ...
    # combine 2 algorithms by taking the min
    # predictions = valData.map(lambda x: ((x[0], x[1]), min(algo.predict(x[0], x[1], r_ui=x[2], verbose=False).est, algo1.predict(x[0], x[1], r_ui=x[2],verbose=False).est)))
